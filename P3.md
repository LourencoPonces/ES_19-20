...
#### Rationale TODO
In the end-to-end tests, we tried to cover the most common use cases of this feature: all the correct procedures (from a Clarification Request submission, to it's answer, and respective edition and deletion), but also the ones that are not permitted, resulting from lack of familiarity with the interface or Quiz Tutor itself. This kind of interactions results in an error and the display of an understandable error message to the user.
