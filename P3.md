...
#### Rationale
We did end to end tests to cover only the basic parts of the functionalities that were implemented. Most of the tests were done in createTournament where the tournaments were created with different attributes. In getAvailableTournaments we tested if the created tournaments were being listed in the right way. At last, in signUpInTournaments it was only made one test where we checked if the tournament created was signed-up, we didn't test the action of signing-up because for that it was required that one student created the test and other to sign up and we were using, always, for all tests, the same user.
