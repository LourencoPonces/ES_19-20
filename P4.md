...
#### F4: _A Student Question can belong to the set of available questions_
...
#### F5: _A Student Question can be edited before belonging to the set of available questions_
...
#### F6: _A rejected question can be corrected and resubmitted by its student_
...
**NOTE:** In order for the feature to run, a `$ psql tutordb < [insert_stats.sql](backend/insert_stats.sql)` needs to be loaded to add stats to already created users.
...
#### Feature number 2: _O docente pode tornar a questão e o seu esclarecimento disponível para os outros alunos_
 ...
 - [x] [Cypress use case test](https://github.com/tecnico-softeng/es20al_19-project/blob/develop/frontend/tests/e2e/specs/teacher/manageClarificationRequests.js)
   + Screenshot: 
     ...
     ![Test results](p4-images/cypress_results.png)
...
**NOTE:** In order for the feature to run, a [sql script](backend/insert_stats.sql) needs to be loaded to add stats to already created users.
 ...
 - [x] [JMeter feature tests](https://github.com/tecnico-softeng/es20al_19-project/blob/develop/backend/jmeter/dashboard/WSgetClarificationRequestDashboardStats.jmx)
   + Screenshots:
     ...
     ![Test results](p4-images/DdP_f6_respBody.png)
